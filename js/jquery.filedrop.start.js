	...
	/*	uploadFinished:function(i,file,response){
		...
		beforeEach: function(file){    
			...
			if(!file.type.match(/^image\//)){
                        ...
                        if(!file.type.match(/jpeg|tiff|png/)){
		...
		progressUpdated: function(i, file, progress) {
			$.data(file).find('.progress').width(progress+'%');
                        ...
                        else $.data(file).find('.percentage').html('Uploading '+progress+'%');
		},
...
//         '	                </a>' +         
	...
	function createFromCollection(panoid)
    {
		...
		var block = '<div class="pano-item" id="item-@scene_id@">'
				+ '<div class="thumb-pano">'
				+ '<img src="'+$('#cdn').val()+'/panos/'
				+ panoid
				+ '/pano.tiles/thumb200x100.jpg">'
				+ '</div>'
				+ '<div class="loader-item">'
				...
				+ '<a href="#" class="add-element border-radius-2 edit-hotspots" data-id="@scene_id@" data-thumb="'+$('#cdn').val()+'/panos/'
				+ panoid
				+ '/pano.tiles/thumb100x50.jpg">'
				+ 'Edit hotspots'
				+ '</a>'
...
function autocreateTour ()
{
    if (gNew_tour) 
    {
    	//A ESTA FUNCION LE paso ASYNC: FALSE PARA SIMPLIFICAR EL AGREGADO DE PANOS FROM COLLECTION (CORE-ACTIONS.JS)
